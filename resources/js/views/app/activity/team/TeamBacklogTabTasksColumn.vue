  ...
  <div class="rounded-lg border border-slate-200 bg-slate-50/50 p-3 min-h-[4rem]">
    ...
    >
      <template #item="{ element: task }">
        <div
          class="rounded-md border border-white bg-white px-2 py-2 shadow-sm flex items-start gap-2 text-sm"
        >
          <span
    ...
    <div v-else class="space-y-2">
      <div
        v-for="task in tasks"
        :key="task.id"
        class="rounded-md border border-white bg-white px-2 py-2 shadow-sm flex items-start justify-between gap-2 text-sm"
      >
        <div class="min-w-0">
          <p class="font-medium text-slate-800 break-words">{{ task.title }}</p>
          <Tag :value="taskStatusLabel(task)" severity="secondary" class="text-xs mt-1" />
